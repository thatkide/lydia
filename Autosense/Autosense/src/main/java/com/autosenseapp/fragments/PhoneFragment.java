...
import com.autosenseapp.activities.SMSConversation;
import com.autosenseapp.databases.MessagesDataSource;
import com.autosenseapp.includes.Helpers;
import com.autosenseapp.includes.PhoneBaseAdapter;
...
public class PhoneFragment extends Fragment {
	...
	private BroadcastReceiver smsReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			final SMS sms = (SMS) intent.getSerializableExtra(Intents.SMSRECEIVED);
			...
			// store message in db
			dataSource.createMessage(message, phoneNumber, com.autosenseapp.databases.Message.TYPE_SMS, false);
			...
			new AlertDialog.Builder(
					...
					.setPositiveButton(getString(R.string.reply), new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface dialogInterface, int i) {
							...
							activity.sendBroadcast(new Intent(Intents.SMSREPLY).putExtra(Intents.SMSRECEIVED, smsReply));
							dialogInterface.cancel();
						}
					}).create().show();
		}
	...
	public BroadcastReceiver incomingCallReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			// store in db
			dataSource.createMessage("", call.getFromNumber(), com.autosenseapp.databases.Message.TYPE_PHONE, false);
			recentCallsView.setAdapter(new PhoneBaseAdapter(activity.getApplicationContext(), dataSource.getAllPhonecalls()));
